<template>
    <Container>
        <button type="button" @click="store.handleCustomInstructions" v-for="(item, index) in sidebarItems" :key="index"
            :class="[
        ...
        <AddItem @click="isOpen = !isOpen" />
    </Container>

    <Teleport to="body">
        <div class="z-[51] fixed inset-0 bg-transparent backdrop-blur-[2px] grid place-items-center" v-if="isOpen">
            <div class="w-full max-w-xl mx-auto bg-chatgpt-600 text-gray-400 rounded-xl divide-y divide-white/10"
                ref="target">
                <div class="px-4 py-4">
