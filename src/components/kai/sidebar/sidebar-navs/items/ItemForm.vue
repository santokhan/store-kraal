<template>
    <form v-if="edit" class="h-12 flex items-center gap-3 px-3 bg-chatgpt-700 rounded-md" @submit="handleSubmit">
        <Chat class="w-4 min-w-[16px]" />
        <input type="text" :value="navText || props.chat.nav" @input="(e: any) => { navText = e.target.value }"
    ...
    <div v-else
        ...
        <Chat class="w-4 min-w-[16px]" />
        <NavText>{{ props.chat.nav }}</NavText>
        <div class="grid grid-cols-3 w-[11rem]">
            ...
            <button @click="store.deleteMessage(props.chat.id, props.chat._id)" type="button"
...
const props = defineProps<{ chat: ChatList }>()
const edit = ref<boolean>(false)
const store = useChatSideBarStore()
// make chat history to reactive state
const navText = ref<string>(props.chat.nav);

function handleSubmit(e: any) {
    e.preventDefault();
    // change the nav text on submit
    store.editMessage(props.chat.id, navText.value, props.chat._id)
