<template>
    ...
    <div v-if="writerIcon" class="w-2 h-5 my-1.5 bg-white/50 on-off"></div>
...
const writerIcon = ref<boolean>(true)
...
function typeWriter(robot: string) {
    ...
    const interval = setInterval(() => {
        if (textArray.length > robotText.length) {
            ...
            // stop on writing start
            writerIcon.value = false
        } else {
            clearInterval(interval)
            // Set `writerIcon.value = false` on typing end
            writerIcon.value = false

            // Invocate hljs.highlightAll() on typewriter end
            // You can call `hljs.highlightAll()` 2 times
            hljs.highlightAll()
        }
    }, typingDelay)
}
