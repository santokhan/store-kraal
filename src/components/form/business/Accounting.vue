<template>
    <div class="relative">
        <Listbox v-model="selectedPerson">
            ...
            <ListboxOptions class="p-1 rounded-lg border border-blue-300 bg-blue-50 text-blue-950 absolute top-full left-0 w-full shadow">
                <ListboxOption v-for="person in people" :key="person.id" :value="person" :disabled="person.unavailable"
                    @click="() => { props.accounting(person.name) }"
...
const props = defineProps<{ accounting: (account: string) => void }>()
...
const people: People[] = [
    { id: 1, name: "QuickBooks", unavailable: false },
    { id: 2, name: "NetSuite", unavailable: false },
    { id: 3, name: "Freshbooks", unavailable: false },
    { id: 4, name: "Intacct", unavailable: true },
    { id: 5, name: "ZohoBooks", unavailable: false },
