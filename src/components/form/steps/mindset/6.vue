...
function nextForm(value: string) {
  formData.updateInvest("b", value);
  formData.$patch({ invest: { b: value } });
  next();
}
...
<template>
  <QContainer>
    ...
    <QMain>
      <QTitle>Have you discussed your financial goals with your kids?</QTitle>
      <QRowButton
        v-for="q in ques"
        :setData="nextForm"
        :value="q"
        :isActive="q == formData.invest.b ? true : false"
      />
    </QMain>
  </QContainer>
</template>
